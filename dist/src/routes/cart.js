"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var express_1 = require("express");
var auth_1 = require("../controller/auth");
var cart_1 = require("../controller/cart");
var jws_1 = require("../api/jws");
var cartRoute = (0, express_1.Router)();
cartRoute.get("/", auth_1.isLogged, jws_1.checkAuth, auth_1.loggedIsNotDestroyed, cart_1.cart);
cartRoute.post("/", auth_1.isLogged, jws_1.checkAuth, auth_1.loggedIsNotDestroyed, cart_1.cartSender);
cartRoute.post("/:id", auth_1.loggedIsNotDestroyed, jws_1.checkAuth, auth_1.isLogged, cart_1.productToCartController);
exports.default = cartRoute;
